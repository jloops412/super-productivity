...
- You choose **Pause & take a break** — Task tracking pauses, a break timer starts, and the reminder counter resets.
...
- **Snooze time** — How long the reminder stays hidden after you snooze. It is also the target duration for a manually started break. Default is often 15 minutes.
...
## Starting and Ending a Manual Break

Choose **Pause & take a break** from the reminder banner. Super Productivity pauses task tracking and shows the break’s remaining time and progress in the banner.

The timer notifies you when it reaches the configured snooze duration, but it continues measuring until you choose **End break** or start tracking a task again. The actual elapsed duration is then recorded as break time for the active work context.
...
The design is **non-punitive**: you can snooze the reminder or pause tracking and take a timed break. For users who want stronger boundaries, the app offers **escalating options** on the **desktop** (Electron) only: **lock screen** and **fullscreen blocker** (with a configurable duration). You choose how strong the nudge is—from a gentle banner to a forced pause.
...
- **Respecting agency** — Snooze lets you postpone the reminder, while **Pause & take a break** begins a timer without replacing an active Focus session.
- **Break time metrics** — A manually started break records its actual elapsed duration when you end it or resume task tracking.
...
Starting a **Focus Mode break resets the break-reminder timer**. A manual break started from the reminder uses its own lightweight timer, so it can pause task tracking without replacing a paused Countdown, Pomodoro, or Flowtime session.
...
- **Only time with a task being tracked** (and certain manual work time) counts; the timer resets when you start a manual or Focus Mode break (or after no task for a while if idle tracking is off).
- You configure **minimum work time** and **snooze time**; snooze time is also the target for manual breaks. You can customize the message and add images.
...
- Manual break tracking is **independent of the Focus timer**, so taking a break does not overwrite a paused Focus session.
